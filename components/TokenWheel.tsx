...
export default function TokenWheel({
  ...
  const handleSpin = useCallback(() => {
    ...
    const targetRotation = calculateTargetRotation(wedges, chosenToken, rotation);
    ...
    setTimeout(() => {
      ...
      onTokenSelect(chosenToken);

      // Reset pointer bounce after animation
      setTimeout(() => setPointerBounce(false), 500);
    }, 4000);
  }, [isSpinning, disabled, wedges, chosenToken, rotation, onTokenSelect]);
  ...
  const handleWedgeClick = useCallback(
    ...
    [isSpinning, disabled, onTokenSelect]
  );
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="relative" style={{ width: size, height: size }}>
        {/* Pointer */}
        <div
          className={`absolute z-10 left-1/2 -translate-x-1/2 -top-1 ${pointerBounce ? 'pointer-bounce' : ''}`}
              ...
              return (
                <g key={wedge.token}>
                  ...
                  <path
                    ...
                    strokeWidth="1.5"
                    style={{
                      filter: isSelected ? 'brightness(1.2)' : isChosen ? 'brightness(1.1)' : 'none',
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isChosen ? 'ring-2 ring-amber-400 ring-offset-1 bg-amber-50 dark:bg-amber-950' : ''}
                  ${
              ...
              >
                <span
                  ...
                  style={{ backgroundColor: WEDGE_COLORS[index % WEDGE_COLORS.length] }}
                />
                <span className="font-mono text-zinc-700 dark:text-zinc-300 truncate max-w-[80px]">
                  {formatTokenForDisplay(wedge.token)}
                </span>
